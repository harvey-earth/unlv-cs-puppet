...
class nrpe (
  ...
  if $ssl_cert_file_content {
    ...
    file { "${nrpe_ssl_dir}/ca_cert.pem":
      ensure  => file,
      owner   => 'root',
      group   => $nrpe_group,
      mode    => '0640',
      source  => $ssl_cacert_file_content,
      notify  => Service[$service_name],
    }
    file { "${nrpe_ssl_dir}/${nrpe_hostname}.pem":
      ensure  => file,
      owner   => 'root',
      group   => $nrpe_group,
      mode    => '0640',
      source  => $ssl_cert_file_content,
      notify  => Service[$service_name],
    }
    file { "${nrpe_ssl_dir}/${nrpe_hostname}.key":
      ensure  => file,
      owner   => 'root',
      group   => $nrpe_group,
      mode    => '0640',
      source  => $ssl_privatekey_file_content,
      notify  => Service[$service_name],
    }
  }
